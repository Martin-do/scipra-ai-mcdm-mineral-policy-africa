...
- Direct reconstructed labels without stance review: **735**
  - Resistant (`0`): **672**
  - Pro-integration (`1`): **63**
- Review queue: **189 records**

The 189-record queue is not identical to 189 unlabeled records: it also includes records whose stance label was clear but whose dominant-stakeholder attribution was low-confidence.
...
### SWDC/dynamic-weighting under-specification

The current executable equation applies a scalar factor:

`W_a = W_0 × (1 + δ × SIC)`

If the same scalar SIC is applied to every criterion and the vector is then normalized, all relative criterion weights return exactly to `W_0`.

Therefore the reported non-trivial changes (for example Local Employment 0.15 → approximately 0.184) require a **criterion-specific stakeholder/sensitivity mapping** or a different normalization rule. That mapping is not encoded in the current implementation, so the full dynamic criterion vector is presently marked **under-specified**, not silently reconstructed by assumption.
...
PR: `SCIPRA dataset reconstruction — frozen replication corpus audit`
...
A packaged freeze bundle is also available at:
...
3. `data/post_freeze_analysis/reconstructed_annotation_draft.csv`
4. `data/post_freeze_analysis/annotation_review_queue.csv`
5. `data/post_freeze_analysis/post_freeze_recovery_status.csv`
6. `data/post_freeze_analysis/mcdm_reproducibility_audit.json`
7. `data/post_freeze_analysis/scenario_formula_validation.csv`
8. `data/post_freeze_analysis/mathematical_audit_hashes.json`
9. `data/post_freeze_analysis/annotation_protocol_excerpt.txt`
...
4. **Ambiguous cases:** Are the fail-closed review rules appropriate, or should more/less of the low-confidence cases be manually adjudicated before fitting the SVM?
5. **MCDM mathematics:** Do you agree that the current scalar-SIC normalization cannot produce non-trivial relative criterion reweighting without an additional criterion-specific mapping?
...
Several manuscript claims reproduce cleanly (notably SIC arithmetic and PCI arithmetic), while other elements show material reproducibility gaps (historical corpus provenance, historical stance distribution, parts of RPCI reporting, and the dynamic-weight implementation).

These discrepancies are being retained as findings rather than corrected by reverse-engineering outputs to match the manuscript.
